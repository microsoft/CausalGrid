...
#If clean repo, might have to make the tbl and fig dirs.
suffix = "-temp"
```
...
est_names_ct = c("Causal Tree (CT CV2)", "Causal Tree - Adaptive (CT-A CV2)", 
                 "Causal Tree (CT CV5)", "Causal Tree - Adaptive (CT-A CV5)")
est_names_cg = c("Causal Grid - Matching CT-A size (CG:M)", "Causal Grid (CG)", 
...
sim_ct_fit <- function(y, X, w, tr_sample, honest=FALSE, cv_folds=nfolds) {
  yX = yX_data(y, X)
  set.seed(my_seed)
  fit = ct_cv_tree("Y~.", data=yX, treatment=w, index_tr=tr_sample, minsize=minsize, split.Bucket=TRUE, bucketNum=b, xval=cv_folds, split.Honest=honest, cv.Honest=honest)
...
pb = utils::txtProgressBar(0, S*NN*D, style = 1)
for(d in 1:D) {
  for(N_i in 1:NN) {
    ...
    for(s in 1:S){
      ...
      fits = list(ct_h2, ct_a2,ct_h5, ct_a5)
      ...
      outer_results_ct[[iter]] = list(s, fits)
    }
  }
}
close(pb)
...
mse_ct_group_mean = apply(mse_ct_group_orig, c(1,2), mean)
mse_ct_group_mean
```
...
for(d in 1:D) {
  for(N_i in 1:NN) {
    ...
    results_s = foreach(s=1:S, .packages=c("proto","gsubfn","rpart", "rpart.plot", "data.table","causalTree", "ranger",  "lattice", "ggplot2", "caret", "Matrix", "foreach", "CausalGrid"), .errorhandling = "pass") %doChange% { #, .combine=rbind
      ...
      ct_a_nl_s = nl_ct_group[dgp_i, 2, s]
      grid_a_am_fit <- change_complexity(grid_a_fit, y, X, d=w, which.min(abs(ct_a_nl_s - (grid_a_fit$complexity_seq + 1))))
      ct_h_nl_s = nl_ct_group[dgp_i, 1, s]
      grid_a_LassoCV_b_hm_fit <- change_complexity(grid_a_LassoCV_b_fit, y, X, d=w, which.min(abs(ct_h_nl_s - (grid_a_LassoCV_b_fit$complexity_seq + 1))))
      grid_a_RF_b_hm_fit <- change_complexity(grid_a_RF_b_fit, y, X, d=w, which.min(abs(ct_h_nl_s - (grid_a_RF_b_fit$complexity_seq + 1))))
      
      grid_a_RF_fit = save_space_cg_rf(grid_a_RF_fit)
      grid_a_RF_b_fit = save_space_cg_rf(grid_a_RF_b_fit)
      fits_base = list(grid_a_fit, grid_a_LassoCV_fit, grid_a_RF_fit, grid_a_LassoCV_b_fit, grid_a_RF_b_fit)
      res_mat = matrix(nrow=0, ncol=4)
      for(fit in c(fits_base, list(grid_a_am_fit, grid_a_LassoCV_b_hm_fit, grid_a_RF_b_hm_fit))) {
    ...
    for(s in 1:S) {
      ...
      #  next
      #}
      list[s_s,fits_base, res_mat] = results_s[[s]]
