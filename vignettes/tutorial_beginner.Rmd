...
X_cell_ids = predict(my_part, new_X=X)
get_desc_df(my_part)
```
...
est_part_subst = fit_estimate_partition(y, X_subst, d, breaks_per_dim=breaks, cv_folds=2)
est_part_subst$interaction_weights
```

A case where there are complimentarities:
```{r}
tau_int = as.integer(X[,1]>0.5 & X[,2]>0.5)*1
y_int = tau_int*d + rnorm(N)
est_part_int = fit_estimate_partition(y_int, X, d, breaks_per_dim=breaks, cv_folds=2)
est_part_int$interaction_weights
```
